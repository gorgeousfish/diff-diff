...
def _run_diff_diff_common(df, rolling, estimator, vce, controls=None, cluster=None):
    ...
    vce_map = {"robust": "hc1", "ols": "classical"}
    dd_vce = vce_map.get(vce, vce) if vce else "classical"

    model = LWDiD(rolling=rolling, estimator=estimator, vce=dd_vce)
    return model.fit(
        df, outcome="y", unit="unit", time="time", treatment="d", controls=controls, cluster=cluster
    )
...
def _run_diff_diff_staggered(
    ...
    """Run diff-diff LWDiD on staggered panel."""
    from diff_diff import LWDiD

    vce_map = {"robust": "hc1", "ols": "classical"}
    dd_vce = vce_map.get(vce, vce) if vce else "classical"

    model = LWDiD(rolling=rolling, estimator=estimator, vce=dd_vce, control_group=control_group)
    return model.fit(
        df,
        outcome="y",
        unit="unit",
...
def test_equivalence_staggered(
    ...
    # SE comparison (may be looser due to aggregation)
    if np.isfinite(ref.se_att) and ref.se_att > 0:
        np.testing.assert_allclose(
            dd.se,
            ref.se_att,
